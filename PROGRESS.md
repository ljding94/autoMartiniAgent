...
- **Benchmark for the paper**: the 8-polymer test set (`docs/TEST_SET.md`, Seonghan) on a graded 2×2×2 stress ladder — validation cases PEO/PMMA/PS have Martini-3 references (PS's is *provisional* → catch-the-error), novel cases PP/PLA/PVP + the two zwitterions PSBMA/PNOMA test de-novo mapping. N=20, single chain, 8 nm water, 200 ns.
...
2. **Validation harness — done** (`agent/validate.py`). `rank`: perturb a good mapping and show the objective degrades monotonically (random-scramble ladder + functional-group splits). `recover`: degrade → run the loop → climb back. Core validation figures at `derived/PSBMA20/{rank,recover}_validation.pdf`. Reruns on PS/PMMA library polymers for the gold-standard version once that AA data arrives.
...
4. **Benchmark + ablation.** The **agentic-vs-deterministic ablation is done** (`agent/ablation.py`): pure objective optimization *games* the metric (an invalid sulfonate split), while the LLM stays chemically valid without any FG rule and is far more evaluation-efficient — the "reasoning matters" figure, and it motivated the FG-integrity rule. The **benchmark** (per-molecule recover/rank/repair vs. baselines) is data-gated — reruns on the PS/PMMA library polymers when that AA data arrives.
...
| 2026-07-26 | **Benchmark test set delivered (Seonghan) — `docs/TEST_SET.md`.** 8 polymers on a graded 2×2×2 stress ladder (Easy/Medium/Hard/Extreme × Validation/Novel): PEO/PP (easy), PMMA/PLA (medium), PS/PVP (hard — rings), PSBMA/PNOMA (extreme — zwitterion). Validation cases (PEO, PMMA, PS) have Martini-3 references: **PMMA is the clean library ground truth we needed for rank/recover**, and **PS's published params are provisional (M2 bonded terms, under revision) → the flagship "does the QA loop flag a questionable target?" case**. All-atom systems built (CHARMM-GUI Polymer Builder for the 6 standard ones; PSBMA/PNOMA in-house by Seonghan); N=20, single chain in an 8 nm water box; **sims running, ~3–5 days to 200 ns**. Unblocks Path-to-publication step 4 (benchmark) once the trajectories land. | done |
| 2026-07-24 | **Rule checker + ablation — the objective is exploitable, chemistry keeps it honest.** `agent/rules.py`: Martini R/S/T sizing, charge/Q-type, bead-count, and **functional-group integrity** (sulfonate/ammonium atom sets must not split; generic SMARTS deferred); error-severity subset is now the loop's hard constraint. `agent/ablation.py`: agentic-vs-deterministic ablation with a **stronger result than hypothesised** — steepest-ascent single-move greedy games the objective down to **0.0217** (below the good mapping) by **splitting the sulfonate** (invalid); FG-constrained greedy is **stuck at 0.0417** (no valid single move improves); the **LLM agent, given no FG rule, finds the valid W2 repair (0.0370) in 8 evals**. So: blind optimization needs explicit chemical rules to stay valid, the LLM gets that free from reasoning, and is far more eval-efficient (greedy used 79). This *motivated* the FG-integrity rule. Figure `derived/PSBMA20/ablation.pdf`. 54 tests. | done |
| 2026-07-24 | **#4 loop shipped + #3 recover/rank validation complete (all on `main`).** `agent/loop.py` — SWE-agent controller + `LLMPolicy` (OpenAI-compatible via `requests`; **OpenRouter**, `OPENROUTER_API_KEY`; opus-4.8 / sonnet-4.6). Verbs tiled per-monomer; hydrogens auto-carry; hill-climb-from-best; Martini ≤4-heavy as a hard constraint (rejection fed back); best-of-N `--restarts` + `--temperature`. **Key unlock: showing bead connectivity in the observation** → opus **autonomously rediscovers the W2 PSBMA repair** (0.0417→0.0370, −11%, the C6/C9 coupled swap) best-of-3. Then `agent/validate.py` — the sim-free validation: **rank** (perturb a good mapping → objective degrades monotonically; scramble×{1,2,4,8} means 0.058/0.067/0.118/0.194; every functional-group split worse; 34/36 perturbations worse) and **recover** (degrade → run loop → mean **107 % of the degradation gap closed**, every split recovered, 3/4 over-recover to the W2 optimum, grouping 77–85 %). Figures: `derived/PSBMA20/{rank,recover}_validation.pdf`. 43 tests. Branches consolidated onto `main` (PRs #2–#4 merged); `.gitignore` drops loop/repair/validate run artifacts. | done |
...
| —          | **#1 Stage**: 8-polymer benchmark from Seonghan (`docs/TEST_SET.md`) — PEO/PP/PMMA/PLA/PS/PVP/PSBMA/PNOMA @ N=20, 200 ns; systems built, sims ~3–5 days out | in progress (sims running) |
| —          | **#1 Stage**: sampling-sufficiency check                   | not started |
| —          | **#2 Process**: classifier + dispatcher (`classify.py`, `dispatch.py`) — small-molecule cold-start | deferred (out of headline scope) |
| —          | **#2 Process**: Martini Mapper interactive-CLI wrapper — small-molecule backend | deferred (out of headline scope) |
| —          | **#2 Process**: cold-start fallback chain (BRICS + naive) — small-molecule | deferred (out of headline scope) |
| —          | **#2 Process**: Martini 3 fragment→type lookup — small-molecule cold-start | deferred (out of headline scope) |
...
| —          | **#3 Evaluation**: Martini-rule checks — R/S/T sizing, charge/Q-type, bead-count, functional-group integrity (`agent/rules.py`) | done (2026-07-24); FG-integrity via per-molecule atom sets (RDKit SMARTS generic version deferred) |
| —          | **#4 Loop**: agentic-vs-deterministic ablation (`agent/ablation.py`) | done (2026-07-24) |
| —          | **#3 Evaluation**: recover+rank validation harness (perturb known-good mapping → R² drops → loop recovers) | done (2026-07-24) |
| —          | **#4 Loop**: mapping-edit verbs (`repair.py`) + Gaussianity-objective evaluator (`evaluate.py`) | done (2026-07-14, LLM-in-the-loop) |
| —          | **#4 Loop**: SWE-agent-style controller (`agent/loop.py`) — verb action space + evaluate feedback + hill-climb-from-best + rules-as-constraints + budget/plateau/submit + best-of-N restarts + OpenRouter LLM policy | done (2026-07-24) |
...
| —          | **Benchmark**: run rank/recover/repair across the 8-polymer test set — PMMA (library ground truth), PS (catch the provisional model), + novel & charged cases | not started (data-gated on sims) |
